...
void TREYFER_encrypt(uint8_t text[static 8]) {
	unsigned i;
	uint8_t t = text[0];
	for (i = 0; i < 8 * NUMROUNDS; ++i) {
		t += key[i % 8];
		t = sbox[t % 256] + text[(i + 1) % 8];
		text[(i + 1) % 8] = t = (t << 1 | t >> 7) % 256; /* ROTL 1 bit */
	}
}
----
...
----
void TREYFER_decrypt(uint8_t text[static 8]) {
   unsigned i;
   uint8_t t, b;
   for (i = 8 * NUMROUNDS; i--; ) {
      t = text[i % 8] + key[i % 8];
      t = sbox[t % 256];
      b = text[(i + 1) % 8];
      b = (b >> 1 | b << 7) % 256; /* Rotate right 1 bit */
      text[(i + 1) % 8]= b - t;
   }
}
----
...
Wenn keine besondere S-Box gebraucht wird (und man auch nicht die vom Autor vorgeschlagene "Standard" S-Box verwenden will), empfehle ich die folgende "Nothing up my sleeve"-sbox: Man re-interpretiert die signifikantesten Bits von Pi als 256 Bytes (mit dem höchstwertigen Bit des ersten Bytes beginnend) und vertauscht ausgehend von einer Identity-Permutation der Reihe nach deren Einträge an der aktuellen Stelle mit jener Stelle deren Index dem aktuellen Bytewert von Pi entspricht.
