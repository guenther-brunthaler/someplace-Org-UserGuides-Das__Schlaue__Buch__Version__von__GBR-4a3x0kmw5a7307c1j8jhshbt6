...
Die Device-Spezifikation

plughw:CARD=b1,DEV=0

ist eine Kurform von

plug:{SLAVE="hw:CARD=b1,DEV=0"}
...
"hw" ist immer ein Hardware-Device. Problem: Diese können meist nur eine kleine Auwahl an Sample-Formaten und Rates ausgeben.

Daher wird das Plugin "plug" dazwischen geschaltet, welches Sample-Rate und Sample-Format der gewünschten Ausgabe in etwas konvertieren kann, das die Audio-Hardware akzeptiert.

Solche Konvertierungen bringen nur bei der Sound-Ein-/Ausgabe etwas. Daher wird der Mixer ("ctl") statt dessen normalerweise ohne Plugins direkt an die Hardware geleitet.
...
Es gibt zwar eine Komponente "alisp" die in ALSA verwendet werden kann, jedoch ist es kein integraler Bestandteil von ALSA. Insbesondere ist es kein Bestandteil der Konfigurationssprache von ".asoundrc".

Es handelt sich vielmehr um ein Plugin zum programmgesteuerten Zugriff auf Interna von ALSA. Es ist vor allem zu Debugging interessant, wird aber kaum je überhaupt verwendet.

Am besten vergessen - es gibt bessere LISP-Dialekte wenn man etwas in LISP programmieren möchte.
