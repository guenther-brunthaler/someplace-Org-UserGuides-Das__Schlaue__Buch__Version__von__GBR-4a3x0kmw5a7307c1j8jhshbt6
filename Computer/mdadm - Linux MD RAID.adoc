...
Even without a proper configuration file, the following usually works with simple configurations like RAID-1:

----
$ mdadm --assemble --scan

$ cat /proc/mdstat # Show running arrays.
----

This will scan for array component drives, derive the RAID configuration from their superblocks, then start the RAIDs found.
...
...
$ mdadm --misc --detail /dev/md0 # "Check Status" shows completion percentage
...
$ grep -s . /sys/block/md0/md/* # Show results, especially "mismatch_cnt".
----
