...
Default Usernet IP configuration
--------------------------------

If one uses QEMU's default userspace network stack, the following IP addresses will be used:

[options="autowidth"]
|===
| 10.0.2.0/24   | Network
| 255.255.255.0 | Network mask
| 10.0.2.15     | Guest VM's IP address
| 10.0.2.2      | Hypervisor host's IP address
| 10.0.2.3      | DNS-server provided by QEMU
|===
...
Offlining and Onlining CPUs
---------------------------

Say you have configured 8 CPUs for your VM, but the current load is low and you want to put all CPUs offline except the first one:

----
$ for c in $(seq 1 7); do echo 0 > /sys/devices/system/cpu/cpu$c/online; done

$ grep . /sys/devices/system/cpu/{online,offline}
/sys/devices/system/cpu/online:0
/sys/devices/system/cpu/offline:1-7
----

By writing a 0 or 1 to `/sys/devices/system/cpu/cpu3/online` you can take the 4th CPU offline or online, respectively.

`/sys/devices/system/cpu/online` shows a read-only overview of the CPUs currently online.

`/sys/devices/system/cpu/offline` does the same for the CPUs currently offline.
